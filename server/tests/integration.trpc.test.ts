...
const aiFixtures = vi.hoisted(() => {
  const architecture = {
    courseTitle: "Mock Course",
    audience: "Learners",
    prerequisites: [],
    learningOutcomes: ["Understand the topic"],
    chapters: [
      {
        title: "Foundations",
        lessons: [
          { id: "lesson-1", title: "Lesson One", summary: "Introduction", dependsOn: [] },
          { id: "lesson-2", title: "Lesson Two", summary: "Practice", dependsOn: ["lesson-1"] },
        ],
      },
    ],
    dependencyMap: [{ fromLessonId: "lesson-1", toLessonId: "lesson-2", reason: "Builds on basics" }],
    glossaryCandidates: ["Graph", "Traversal"],
    finalProjectConcept: "Mini capstone",
    courseComplexity: "generic" as const,
  };

  const course = {
    title: "Mock Course",
    description: "Generated by the test provider",
    chapters: [
      {
        title: "Foundations",
        ...
        lessons: [
          {
            ...
            glossaryTerms: [{ term: "Graph", definition: "A collection of nodes and edges." }],
            relatedTopics: [{ title: "Trees", description: "A specialized graph." }],
          },
          {
            title: "Lesson Two",
            lessonType: "practice",
            content: "Lesson two content",
            glossaryTerms: [{ term: "Traversal", definition: "Systematically visiting graph nodes." }],
            relatedTopics: [{ title: "Paths", description: "Ways to move through a graph." }],
          },
        ],
      },
    ],
    ...
    questions: [
      {
        ...
        options: ["A data structure", "A database", "A compiler", "A package manager"],
        correctAnswer: "A data structure",
        explanation: "Graphs model nodes and edges.",
      },
    ],
  };
  ...
  return {
    provider: {
      generateCourseArchitecture: vi.fn().mockResolvedValue(architecture),
      generateCourseFromArchitecture: vi.fn().mockResolvedValue(course),
      generateCourse: vi.fn().mockResolvedValue(course),
      generateQuiz: vi.fn().mockResolvedValue(quiz),
      chat: vi.fn().mockResolvedValue({ message: "Mock response" }),
      regenerateLesson: vi.fn().mockResolvedValue({ content: "Regenerated content" }),
    },
  };
});

vi.mock("../ai/factory", () => ({
  createAIProvider: vi.fn(() => aiFixtures.provider),
}));
...
beforeEach(() => {
  Object.values(aiFixtures.provider).forEach((fn) => {
    if ("mockClear" in fn) {
      fn.mockClear();
    }
  });
